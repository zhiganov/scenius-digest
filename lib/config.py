...
def get_all_event_groups() -> dict:
    ...
    # adds its event feeds. Keep those feeds even when Community Admin represents
    # "none configured there" as an empty list. A plain update() replaced the
    # whole entry, so a private community that also had an event source lost
    # `visibility` and became public in /api/events.
    for key, source in EVENT_SOURCES.items():
        ...
        event_apis = []
        for api in [*(source.get("event_apis") or []), *(community.get("event_apis") or [])]:
            if api not in event_apis:
                event_apis.append(api)
        if "event_apis" in source or "event_apis" in community:
            combined["event_apis"] = event_apis
        merged[key] = combined
    return merged
